...
// Network timeouts (in ms) — a stuck upstream call must never hang the whole
// request. Fast failure means the frontend can retry immediately instead of
// spinning for 30+ seconds waiting on TCP.
...
// ─────────────────────────────────────────────────────────────
// fetch-with-timeout helper. Uses AbortController so a hung
// upstream socket cannot pin an HTTP request open forever.
// ─────────────────────────────────────────────────────────────
...
async function getAccessToken() {
  ...
  // Coalesce concurrent token requests so 3 parallel purchases don't
  // each hit /token — they share one in-flight promise.
...
async function initiateSTKPush({ phone, amount, accountRef, description }) {
  ...
  let res, data;
  try {
    res = await fetchWithTimeout(KCB_STK_ENDPOINT, {
      method: "POST",
      headers: {
    ...
    // If the token was rejected mid-flight, drop it so the next call re-authenticates.
    cachedToken = null;
    cachedTokenExpiry = 0;
    const msg = e.name === "AbortError"
      ? "Payment gateway timed out. Please try again."
      : `Network error contacting payment gateway: ${e.message}`;
  ...
  if (!ok) {
    // 401 → invalidate cached token so the next attempt re-authenticates.
    if (res.status === 401) {
      cachedToken = null;
      cachedTokenExpiry = 0;
    }
...
function handleCallback(body) {
  ...
  // M-Pesa result code 1032 = customer pressed Cancel on their phone.
  // Also treat any description containing the word "cancel" as a cancel
  // so we can surface a clean, user-friendly message.
  const isCancelled = resultCode === 1032 || /cancel/i.test(resultDesc || "");
  const finalMessage = isCancelled
    ? "Payment cancelled."
    : (resultDesc || "Payment was not completed");

  if (record) {
    record.status = "FAILED";
    record.failureReason = finalMessage;
 ...
 *
 * NOTE: intervalMs reduced from 2000 → 800ms so the moment the KCB
 * callback lands, the caller returns almost immediately instead of
 * waiting up to 2s more. This is the single biggest reason the
 * previous version felt "very slow after entering the PIN".
 */
async function waitForSettlement(checkoutId, { timeoutMs = 90_000, intervalMs = 800 } = {}) {
...
module.exports = {
  ...
  _config: {
    ...
    KCB_CALLBACK_URL,
    KCB_SHORTCODE,
    KCB_PASSKEY_SET: Boolean(KCB_PASSKEY),
  },
};
